...
object RandomFrames extends TestGenerator("genrmf") {
  override def generate(vertices: Int, edges: Int, maxCapacity: Int): Result = {
    ...
    val layerEdges = for {
      ...
      case 0 => EdgeRec(off + fc + a * sc,        off + fc + a * (sc + 1), inLayerCap)
      case 1 => EdgeRec(off + fc + a * (sc + 1),  off + fc + a * sc,       inLayerCap)
      case 2 => EdgeRec(off + a * fc + sc,        off + a * fc + sc + 1,   inLayerCap)
      case 3 => EdgeRec(off + a * fc + sc + 1,    off + a * fc + sc,       inLayerCap)
    }
    ...
    val crossEdges = for {
      ...
      index <- 0 until a * a
    } yield EdgeRec((layer - 1) * a * a + index, layer * a * a + perm(index), rng.nextInt(c2) + 1)
