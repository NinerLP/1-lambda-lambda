...
import ru.ifmo.ctd.ngp.demo.util.stats.{Util => SUtil, Kruskal}
...
object Summary extends App {
  object RedirectOut {
    def apply(name: String)(code: => Any) {
      ...
      try {
        ...
        scala.Console.setOut(newStream)
        code
        newStream.close()
      } finally {
        scala.Console.setOut(oldOut)
      }
    }
  }
  ...
  val dimacsFiles = {
    ...
    collect(new File("misc/dimacs-tests"), seq += _)
    seq.result().sorted
  }
  ...
  val cacheFile = new File(s".summary-cache-${dimacsFiles.length}")
  ...
  def rankSumComponents(): Unit = {
    def blowUp(a: Seq[Long]) = if (a.size > 1) a else IndexedSeq.fill(200)(a(0))

    for ((algo, res) <- results.groupBy(_.algorithm)) {
      val classMap = res.groupBy(_.testClass).mapValues(_.map(_.edgeCount))
      val classNames = classMap.keys.toIndexedSeq
      val classValues = classNames.map(t => blowUp(classMap(t)))
      val classMedians = classValues.map(med)
      val n = classValues.size

      val ordered = (0 until n).sortBy(t => -classMedians(t))

      def issue(order: Int, collected: IndexedSeq[Int]) {
        val currValues = collected.map(classValues)
        val p = Kruskal.rankSumTest(currValues)
        val m = med(currValues.flatten)
        println(s"  $order: med = $m, p = $p")
        println(collected.map(classNames).sorted.mkString("    ", "\n    ", "\n"))
      }

      def greedyByKruskalTest(idx: Int, order: Int, collected: IndexedSeq[Int]) {
        if (idx == n) {
          issue(order, collected)
        } else {
          val nc = collected :+ ordered(idx)
          if (!(Kruskal.rankSumTest(nc.map(classValues)) < 0.01)) {
            greedyByKruskalTest(idx + 1, order, nc)
          } else {
            issue(order, collected)
            greedyByKruskalTest(idx + 1, order + 1, IndexedSeq(ordered(idx)))
          }
        }
      }

      println(algo)
      greedyByKruskalTest(0, 1, IndexedSeq.empty)
    }
  }
  ...
  RedirectOut("classTable.tex")(classTable())
  RedirectOut("totalTable.tex")(totalTable())
  RedirectOut("correlationCoefficients.tex")(correlationCoefficients())
  RedirectOut("rankSumComponents.txt")(rankSumComponents())
  RedirectOut("classAverageRank.txt")(classAverageRank())
}
