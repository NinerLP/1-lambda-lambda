 ...
 */
object GeneticsBig extends App {
  val maxFitnessEvaluations = 10000000
  ...
  val logRoot = new File("genetic-big-flow-logs")
  logRoot.mkdirs()
  val dimacsRoot = new File("misc/dimacs-tests/tests/genetic-big")
  dimacsRoot.mkdirs()
  ...
  val runs = for {
    ...
    if (dimacsFile.exists()) {
      println(s"File ${dimacsFile.getName} exists and contains ${maxFitnessEvaluations}th evaluation, skipping")
      emptyRun
    } else () => {
